...
Each Codex account renders canonical `5h` (300-minute) and `wk` (10,080-minute) quota windows, classified from the API's normalized `windowMinutes` rather than primary/secondary position. Missing or unknown-duration windows render as `?`; the footer never relabels an unclassified metric as either quota. Percentages remain **remaining quota**, and reset countdowns retain their existing semantics.
