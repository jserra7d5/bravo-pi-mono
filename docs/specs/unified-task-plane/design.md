...
4. **One task plane, one owner.** Agents reason by analogy across tools;
   ...
   management pair (`managed_task_list`, `managed_task_stop`), and one error tier vocabulary — and
...
| `managed_task_stop` | stop a task of either type | `background_task_stop`, `monitor_stop` |
