...
- [x] `task_list` lifecycle visibility
...
- Smoke: foreground bash, owned background bash, self-terminating stream monitor, notifications, and `task_list(include_completed:true)` all worked. Runtime metadata and output files agreed with the agent transcript.
...
Follow-up applied: the shared task-plane guidance now teaches that waiting is an idle state, preserves useful independent work, and tells the agent to end the current response instead of calling `task_list`, sleeping, or polling merely to keep the turn alive.
