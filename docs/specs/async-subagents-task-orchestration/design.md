...
- **Type-appropriate next actions.** Task wakeups now suggest the concrete next action (`subagent_start` for ready, `task_get({ view: "receipt" })` for a submitted result so the parent reviews before accepting/reopening, `task_get` for failures/blockers) instead of always pointing at `task_get`. `task_create` likewise points its `next` at `subagent_start` for each ready task rather than back at `task_list`.
