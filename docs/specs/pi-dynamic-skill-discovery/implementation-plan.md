...
Implement `@bravo/dynamic-skills` as a no-core-mod Pi extension that listens for successful `read` tool results below the session `cwd`, discovers eligible `.agents/skills/*/SKILL.md` and `.claude/skills/*/SKILL.md` files along the touched subtree path, appends a compact skill catalog to the current read result, persists branch-scoped discovery snapshots, and reinjects the compact catalog on future turns. Keep the implementation clean and direct: no native Pi registry mutation, no `/skill:name` dynamic autocomplete in v1, no startup full-tree scan, and no discovery from tools other than `read`.
...
1. `src/scanner.ts` — safe path resolution, cwd containment, upward `.agents/skills` and `.claude/skills` root discovery, skill metadata loading, and security checks.
...
3. **Define durable data types**
   - Implement/export:
     ...
     ```
   - Keep `location` as the absolute `SKILL.md` path. Define `baseDir` as `dirname(location)`, the individual skill directory. If the `.agents/skills` and `.claude/skills` root is needed for loader grouping or diagnostics, add a separate `skillsRoot` field instead of overloading `baseDir`.
...
4. **Implement path and filesystem safety primitives**
   ...
   - Before invoking Pi metadata loading, traverse candidates with `lstat`: `.agents`, `.agents/skills` and `.claude/skills`, each child skill directory, and each `SKILL.md` must be ordinary directories/files as appropriate, not symlinks. Reject and diagnose symlinked roots, skill directories, or `SKILL.md` files.
   - For every `.agents/skills` and `.claude/skills` candidate, individual skill directory, and `SKILL.md`, canonicalize and enforce containment under real `cwd`.
...
5. **Implement skill metadata loading**
   ...
   - After symlink-safe `lstat` traversal, call `loadSkillsFromDir` only on safe individual skill directories, not on an unfiltered `.agents/skills` and `.claude/skills` root that may contain unsafe entries.
...
6. **Implement collision filtering**
   ...
   - Dynamic collision policy:
     ...
     - if a restored legacy snapshot skill or candidate lacks comparable `skillMtimeMs`/source-root data, keep the existing accepted skill and record a duplicate-name diagnostic instead of replacing it.
...
| Path traversal and symlinks | Discovery never scans above real `cwd` and never accepts `.agents/skills` and `.claude/skills` or `SKILL.md` symlink escapes outside real `cwd`. | Scanner tests using `mkdtemp`, real directories, real symlinks, and `realpath`. | Symlinked `.agents/skills` and `.claude/skills` to outside temp dir, symlinked `SKILL.md` outside cwd, path containing `..`, missing target. |
| Upward scope | A deeper read discovers skills from `.agents/skills` and `.claude/skills` roots on the path from target directory up to `cwd`, inclusive, and not from sibling subtrees. | Scanner test with multiple nested directories and sibling skill roots. | Skills in sibling directory, root at cwd, nested root nearer target, duplicate root encountered. |
| Skill metadata semantics | Catalog entries match Pi skill semantics for name, description, location, and `disable-model-invocation`; missing/invalid/disabled skills are excluded. | Call Pi's real exported skill loader against temp `.agents/skills` and `.claude/skills` directories. | Missing description, invalid YAML/frontmatter, disabled model invocation, non-directory entry, unreadable `SKILL.md`. |
| Collision policy | Native skills win; duplicate dynamic names choose newest SKILL.md mtime, with exact ties preferring .agents; no aliases are created. | `before_agent_start` and `tool_result` harness with Pi-shaped native `systemPromptOptions.skills`. | Native same path, native same name different path, two dynamic same names, collision-only discovery. |
...
- **Base directory meaning:** `DynamicSkill.baseDir` is pinned to `dirname(location)` (the individual skill directory). Add `skillsRoot` separately if implementation needs the containing `.agents/skills` and `.claude/skills` root.
