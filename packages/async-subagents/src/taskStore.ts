...
import { deriveTaskState, isTaskReady } from "./taskState.js";
...
function hasSubstantiveValue(value: unknown): boolean {
  ...
  if (Array.isArray(value)) return value.some((item) => hasSubstantiveValue(item));
  if (typeof value === "object") return Object.keys(value).some((key) => hasSubstantiveValue((value as Record<string, unknown>)[key]));
  return true;
}
...
export class TaskStore {
  ...
  createTasks(rootSessionId: string, input: CreateTasksInput): CreateTasksResult { return this.withLock(rootSessionId, "create", () => {
    ...
    // Forward-progress nudge: a task with no unresolved dependencies is ready to
    // run now. Wake the parent so a created-then-idle session does not leave
    // ready tasks sitting forever; the wakeup is deduped/skipped if the task is
    // already claimed by poll time. Blocked tasks stay silent until accept time.
    for (const task of records) { if (isTaskReady(task, all)) this.appendTaskEvent(rootSessionId, task.parentRunId, task.id, "task.ready", `Task ready to start: ${task.id} ${task.title}`, { wake: true }); }
  ...
  submitResult(rootSessionId: string, taskId: string, input: SubmitTaskResultInput): TaskRecord { return this.withLock(rootSessionId, "submit", () => { const task = this.readTaskRaw(rootSessionId, taskId); this.assertOwned(task, input.runId, input.taskToken); const summary = input.summary.trim(); if (!summary) throw new SubagentError("EMPTY_TASK_RESULT", "summary is required"); const paths = this.pathsFor(rootSessionId); const artifactPaths = input.artifactPaths?.map((p) => { if (typeof p !== "string" || !p.trim()) throw new SubagentError("TASK_RESULT_RECEIPT_REQUIRED", "artifactPaths entries must be nonblank strings"); ensureUnder(p, [paths.artifactsDir, this.cwd]); return p; }); const hasInlineReceipt = hasSubstantiveValue(input.receipt); const hasArtifact = Boolean(artifactPaths?.length); const hasEvidence = Boolean(input.evidence?.some((item) => typeof item === "string" && item.trim())); const hasCommand = Boolean(input.commandsRun?.some((item) => typeof item === "string" && item.trim())); const hasNotes = typeof input.notes === "string" && Boolean(input.notes.trim()); if (!hasInlineReceipt && !hasArtifact && !hasEvidence && !hasCommand && !hasNotes) throw new SubagentError("TASK_RESULT_RECEIPT_REQUIRED", "task_submit_result requires a durable receipt or substantive payload: receipt, artifactPaths, evidence, commandsRun, or notes"); let receiptPath: string | undefined; if (hasInlineReceipt) { const bytes = Buffer.byteLength(JSON.stringify(input.receipt), "utf8"); if (bytes > RECEIPT_MAX_BYTES) throw new SubagentError("TASK_RECEIPT_TOO_LARGE", "receipt exceeds 32KB"); receiptPath = join(paths.receiptsDir, `${taskId}-${input.runId}.json`); atomicWriteJson(receiptPath, { schemaVersion: SCHEMA_VERSION, taskId, runId: input.runId, submittedAt: nowIso(), receipt: input.receipt }); }
    const result: TaskResultReceipt = { state: "submitted", summary, receiptPath, artifactPaths, evidence: input.evidence, commandsRun: input.commandsRun, notes: input.notes, submittedAt: nowIso() }; const updated = { ...task, status: "result_ready" as const, result, attempts: task.attempts.map((attempt) => attempt.runId === input.runId && !attempt.endedAt ? { ...attempt, endedAt: result.submittedAt, status: "result_ready" as const } : attempt), updatedAt: nowIso() }; this.writeTask(rootSessionId, updated); this.appendTaskEvent(rootSessionId, task.parentRunId, taskId, "task.result_submitted", summary, { runId: input.runId, wake: true, data: { receiptPath, hasInlineReceipt, artifactPaths, evidence: input.evidence, commandsRun: input.commandsRun } }); return updated; }); }
