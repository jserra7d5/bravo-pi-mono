...
Pi agent definitions encode their normal thinking level. Omit \`thinkingLevel\` by default for every role. Task risk, importance, size, or category does not by itself justify raising it. Override upward only for a narrow, named reasoning bottleneck that the configured default is unlikely to handle and that cannot be removed through a clearer brief, tighter scope, better tooling, or stronger validation; state that bottleneck before applying the override. Treat high reasoning as a scarce exception, not a general quality setting. Lower the level when a mechanical task does not need the configured budget. Claude variants use \`effort\` instead of \`thinkingLevel\`; do not infer Claude tools from the base Pi agent.
...
5. Omit thinking overrides by default for every role; override upward only for a narrow, named reasoning bottleneck that cannot be addressed with clearer scope, tools, or validation.
