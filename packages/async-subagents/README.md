...
Task storage lives next to run delivery state under `session-tasks/<rootSessionId>/`. Tool returns are compact by default: `task_get` starts with status/pointers and only returns receipt/full details when requested.
