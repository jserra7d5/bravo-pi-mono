...
test("async activity state derivation treats nonterminal children as active and blocked rows as blocked", () => {
  ...
  assert.deepEqual(deriveAsyncSubagentsActivityState({ rows: snapshot.rows }), {
    active: true,
    blocked: true,
    activeCount: 2,
    message: "needs input",
...
test("async activity state derivation keeps only current terminal result-ready rows active", () => {
  ...
  assert.equal(deriveAsyncSubagentsActivityState({ rows: snapshot.rows }).activeCount, 1);
});
...
test("async activity state keeps old uncollected terminal result-ready rows active", () => {
  ...
  const state = readAsyncSubagentsActivityState({ store: w.store, parentRunId: w.parentRunId, rootSessionId: w.parentRunId });
  assert.equal(state.active, true);
  assert.equal(state.blocked, false);
  assert.equal(state.activeCount, 1);
});

test("async activity state does not keep handled terminal results active", () => {
  const w = workspace();
...
test("async activity state reconciles dead process-owned rows before deriving active state", () => {
  ...
  const state = readAsyncSubagentsActivityState({
    store: w.store,
