...
- One notification when something you own finishes → bash({command, run_in_background: true}). Tests, builds, installs, migrations, dev servers, scripts. Use when you own a long-running process and care about its single completion; avoid when observing external evidence or when the command finishes in seconds. You'll be notified when it exits — do not poll managed_task_list, do not append &.
...
Waiting is an idle state, not a tool call. After starting background bash or a monitor, if no independent work remains, end the current response; the task plane will asynchronously wake this session with the notification. Do not call managed_task_list, sleep, or poll merely to keep the turn alive.
