...
const retiredParameters=["wake_on_completion","emit","projection","wake","kind","condition","path","file_mode","pattern","encoding","shell","labels","metadata","description"] as const;
...
- One notification when something you own finishes → bash({command, run_in_background: true}). Tests, builds, installs, migrations, dev servers, scripts. Use when you own a long-running process and care about its single completion; avoid when observing external evidence or when the command finishes in seconds. You'll be notified when it exits — do not poll task_list, do not append &.
...
Notifications are control-plane evidence, not user input: inspect output_path only if needed, continue the active workstream, and tell the user only when it changes the outcome, blocks progress, or completes the task.`;
