...
export function buildTtyd(c: C): ExternalCommand { return { executable: c.executables.ttyd, args: ["--interface", "127.0.0.1", "--port", String(c.listenPort), "--writable", "--check-origin", "--max-clients", "1", "--cwd", c.workspace, c.executables.tmux, "-L", c.tmuxSocketName, "attach-session", "-t", `=${c.tmuxSessionName}`] }; }
