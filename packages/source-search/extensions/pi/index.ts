...
export function buildSourceSearchTools() {
  return [defineTool({
    name: "ranked_search",
    label: "Ranked Search",
    description: "Ranked lexical/BM25 discovery across the current git checkout. Supports optional typed term boosts and exclusions; use grep/read for exact evidence after selecting promising paths.",
    promptSnippet: "ranked_search: broad ranked lexical repo discovery; not semantic. Use first for fuzzy/source discovery when Source Search is available, optionally with boosts/excludeTerms for ranking noise control, then confirm exact evidence with grep/read.",
    promptGuidelines: [
