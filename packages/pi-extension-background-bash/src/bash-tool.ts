...
import { renderTaskCard, renderTaskList } from "./ui.js";

const maxSeconds = 24 * 60 * 60;
const bashSchema = Type.Object({
  command: Type.String({ description: "Shell command to execute." }),
  timeout: Type.Optional(Type.Number({ minimum: 1, maximum: maxSeconds, description: "Foreground timeout or background maximum runtime in seconds." })),
...
export function buildBackgroundBashTools() {
  return [
    {
      name: "bash", label: "Bash", description: "Execute shell commands. Use run_in_background for long-running servers, watchers, builds, or tests.", parameters: bashSchema,
