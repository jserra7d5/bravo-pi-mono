...
export class AskUserQuestionComponent implements Component {
  ...
  private theme: Theme;
  private tui: TUILike;
  private done: (result: PickerOutcome) => void;

  private states: QuestionState[];
  private activeTab: number = 0;
  private editor: Editor;
  private escapeLabel: "close" | "cancel";
  ...
  private renderTabBar(_width: number, add: (s: string) => void): void {
    ...
    for (let i = 0; i < this.questions.length; i++) {
      ...
      // Keep tabs compact even though tool input permits longer headers.
  ...
  private renderQuestionBody(
    ...
    if (state.inEditMode) {
      ...
      add(t.fg("dim", ` ↑↓ navigate · ${actionHint}${tabHint} · Esc ${this.escapeLabel}`));
    }
  }
  ...
  private renderSubmitTab(_width: number, add: (s: string) => void): void {
    ...
    add("");
    add(t.fg("dim", ` ←→ switch tabs · Esc ${this.escapeLabel}`));
  }
  ...
  private advance(): void {
    if (this.activeTab < this.questions.length - 1) {
      ...
      // Selection records the draft; submission is always a separate action
      // from the explicit review screen, including for a single question.
  ...
  private submit(): void {
    this._resolved = true;
    this.done({ kind: "submitted", answers: this.buildAnswers() });
  }
  ...
  private buildAnswers(): Answer[] {
    return this.questions.map((q, i) => {
      const s = this.states[i];
      const indices = q.multiSelect ? [...s.selectedIndices].sort((a, b) => a - b) : s.selectedIndex === null ? [] : [s.selectedIndex];
      return {
        question: q.question,
        selected_option_ids: indices.map((index) => q.options[index].id ?? `option-${i}-${index}`),
        selected_labels: indices.map((index) => q.options[index].label),
        ...(s.freeTextValue !== null ? { free_text: s.freeTextValue } : {}),
      };
    });
  }
