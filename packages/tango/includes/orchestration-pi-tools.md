...
- `tango_cli` is the generic CLI escape hatch for safe Tango commands and flags not yet exposed by a dedicated tool. Long-running commands such as `tango watch` are intentionally not exposed through `tango_cli`.
...
Parent Pi sessions may receive proactive notifications when child agents finish, block, or error. Treat notifications as prompts to inspect the child; always inspect child output with `tango_look` or `tango_result` before summarizing a child agent's work.
