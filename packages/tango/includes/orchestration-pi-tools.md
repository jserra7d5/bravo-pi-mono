...
`tango_start` returns immediately. Parent Pi sessions may receive proactive wake-ups for unresolved Tango inbox items such as blocked/error children, stalled/offline children, and ready results. In Pi these wake-ups are delivered as custom `tango-message` UI messages backed by inbox metadata; non-Pi/agent-directed structured messages use the `---BEGIN TANGO MESSAGE---` / `---END TANGO MESSAGE---` text envelope. Mark inbox items handled or dismissed with `tango_inbox` (or collect/read results with the result tools) to suppress further notifications for those items. Treat wake-ups as prompts to inspect the child; always inspect child output with `tango_activity` or `tango_result` before summarizing a child agent's work.
